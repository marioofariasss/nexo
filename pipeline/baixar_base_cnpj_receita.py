...
def requisicao(url: str, token: str, method: str = "GET", depth: int | None = None):
...
def baixar(url: str, destino: Path, token: str, tamanho_esperado: int, sobrescrever: bool) -> None:
    ...
    with urllib.request.urlopen(requisicao(url, token), timeout=180) as origem, temporario.open("wb") as saida:
...
def main() -> None:
    ...
    if args.listar:
        ...
        return
    for nome, tamanho in arquivos:
        url = f"{BASE}{snapshot}/{urllib.parse.quote(nome)}"
        baixar(url, pasta / nome, args.share_token, tamanho, args.sobrescrever)
    print("Pronto:", pasta)
